...
# sys.path.insert(0, r'C:\Users\Admin\OneDrive\Desktop\yes')
...
# @st.cache_resource
# def load_engine():
#     return RiskEngine(base_dir=r'C:\Users\Admin\OneDrive\Desktop\yes')
...
# # Realistic random value ranges per dataset field
# RANDOM_RANGES = {
#     # DS1
...
#     'NumberOfDependents': (0, 8),
#     # DS2
#     'total_open_amount': (100, 500000),
#     'posting_dayofyear': (1, 365),
#     'posting_year': (2018, 2024),
#     'cust_payment_terms': (0, 90),
#     'due_in_date': (1, 120),
#     'invoice_id': (100000, 999999),
#     'converted_usd': (100, 500000),
#     'document_date': (1, 365),
#     # DS3
#     'Attr1': (0.0, 5.0),
#     'Attr2': (0.0, 5.0),
...
#     'Attr10': (0.0, 5.0),
#     # DS4
#     'ROA(C) before interest and depreciation before interest': (-0.5, 0.5),
#     'Operating Gross Margin': (-0.5, 1.0),
#     'Realized Sales Gross Margin': (-0.5, 1.0),
#     'Operating Profit Rate': (-0.5, 1.0),
#     'Pre-tax net Interest Rate': (-0.5, 0.5),
#     'After-tax net Interest Rate': (-0.5, 0.5),
#     'Non-industry income and expenditure/revenue': (-0.5, 0.5),
#     'Continuous interest rate (after tax)': (-0.5, 0.5),
#     # DS5
#     'duration': (6, 72),
#     'credit_amount': (500, 20000),
...
# INTEGER_FIELDS = {
#     'age', 'NumberOfTime30-59DaysPastDueNotWorse', 'NumberOfOpenCreditLinesAndLoans',
#     'NumberOfTimes90DaysLate', 'NumberRealEstateLoansOrLines',
#     'NumberOfTime60-89DaysPastDueNotWorse', 'NumberOfDependents',
#     'posting_dayofyear', 'posting_year', 'cust_payment_terms', 'due_in_date',
#     'invoice_id', 'document_date',
#     'duration', 'installment_rate', 'present_residence',
...
#     sample = {}
#     for field in fields:
#         lo, hi = RANDOM_RANGES.get(field, (0.0, 1.0))
#         if field in INTEGER_FIELDS:
#             sample[field] = str(random.randint(int(lo), int(hi)))
#         else:
#             sample[field] = str(round(random.uniform(lo, hi), 4))
#     return sample
...
#     # Initialise session state for random sample
#     if 'random_sample' not in st.session_state:
#         st.session_state.random_sample = {}
#     if 'last_ds' not in st.session_state:
#         st.session_state.last_ds = ds_choice

#     # Reset sample when dataset changes
#     if st.session_state.last_ds != ds_choice:
#         st.session_state.random_sample = {}
#         st.session_state.last_ds = ds_choice
...
#             st.session_state.random_sample = generate_random_sample(fields)
...
#             prefill = st.session_state.random_sample.get(field, '')
#             val = col.text_input(field, value=prefill, key=f'field_{ds_choice}_{field}')
#             if val.strip() != '':
#                 try:
...
#             insuff = sum(1 for r in results if r['verdict'] == 'INSUFFICIENT_DATA')
